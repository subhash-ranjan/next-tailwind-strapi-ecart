...
const Index = () => {
    ...
    const scrollFunction = () => {
        if (myRef.current) {
            if (document.documentElement.scrollTop > 100 && myRef.current.classList.contains('hidden')) {
                myRef.current.classList.remove('hidden');
                myRef.current.classList.add('fixed');
                dispatch({ type: 'IS_SCROLLED', payload: true });
            }

            if (document.documentElement.scrollTop < 100 && myRef.current.classList.contains('fixed')) {
    ...
    return (
        <div className="bg-gray-800 flex-col-center-center h-28 w-full ">
            <div
                ref={myRef}
                className="hidden  cursor-pointer rounded-full
				 bg-indigo-600  opacity-80 
                 p-3 md:p-5 lg:p-3
                 w-10 h-10 md:w-16 md:h-16  lg:w-12 lg:h-12
                 bottom-5 right-5 md:bottom-10 md:right-10  lg:bottom-7 lg:right-7"
                onClick={() => {
                    scrollTop();
                }}
            >
            ...
            <div className="flex w-12 flex-row items-center justify-between">
                <a target="_blank" rel="noreferrer" href="https://www.github.com/subhash-ranjan">
            ...
            <span className="my-1 text-sm text-gray-400"> Copyright - S.R. | All Rights Reserved</span>
        </div>
    );
};
