...
const Index = ({ data }: { data: CategoryType[] }) => {
    ...
    return (
        <div
            id="dv-home"
            className={`bg-gray-100  min-h-[100vh] size-full flex-col-reverse-end-center lg:flex-row-center-start px-5 py-8 lg:px-10`}
        >
            ...
            <div className="size-full lg:min-h-[80vh] grid-col-2 lg:flex-row-center-center">
                {Array.from([1, 2, 3]).map((item, index1) => {
                    return (
                        <div key={index1} className="flex grid-rows-2 lg:flex-col-center-center">
                            {Array.from([1, 2]).map((item, index2) => {
                                const height =
                                    [1, 2, 5].indexOf(index) >= 0
                                        ? 'h-[32vmin] lg:h-[22vmin]'
                                        : 'h-[32vmin] lg:h-[32vmin]';

                                const url = `/books/${lstCategory[index].attributes.imgurl}.jpg`;
